...
namespace QuanLyQuanCaPhe.Forms
{
    public partial class frmQuanLiBan : Form
    {
            ...
            bool isEmbedded = false)
        {
            ...
            _btnQuanLyKhoSidebar.Click += (_, _) => FormNavigationService.Navigate(this, _permissionBUS, PermissionFeatures.NguyenLieu, () => new frmQuanLiKho(isEmbedded: true), onCurrentFormReactivated: RefreshView, skipNavigation: _isEmbedded);
            _btnKhachHangSidebar.Click += (_, _) => FormNavigationService.Navigate(this, _permissionBUS, PermissionFeatures.KhachHang, () => new frmKhachHang(isEmbedded: true), onCurrentFormReactivated: RefreshView, skipNavigation: _isEmbedded);
            _btnAuditLogSidebar.Click += (_, _) => FormNavigationService.Navigate(this, _permissionBUS, PermissionFeatures.ThongKe, () => new frmAuditLog(isEmbedded: true), onCurrentFormReactivated: RefreshView, skipNavigation: _isEmbedded);
        ...
        private async void btnThemBan_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                result = await Task.Run(() => _banBUS.ThemBan(tenBan));
            }
            finally
            {
                KetThucXuLyQuanLiBan();
            }
        ...
        private async void btnXoaBan_Click(object? sender, EventArgs e)
        {
            ...
            try
            {
                result = await Task.Run(() => _banBUS.XoaBan(ban.ID));
            }
            finally
            {
                KetThucXuLyQuanLiBan();
        ...
        private bool ThuBatDauXuLyQuanLiBan()
        {
            if (_dangXuLyQuanLiBan)
            {
                return false;
            ...
            _dangXuLyQuanLiBan = true;
            DatTrangThaiDangXuLyQuanLiBan(true);
            return true;
        }

        private void KetThucXuLyQuanLiBan()
        {
            _dangXuLyQuanLiBan = false;
            DatTrangThaiDangXuLyQuanLiBan(false);
        }

        private void DatTrangThaiDangXuLyQuanLiBan(bool dangXuLy)
        {
